...
export function pan(value, SVGDeltaX, SVGDeltaY) {
  ...
  // apply pan limits
  if (panLimit) {
    var _applyToPoints = applyToPoints(matrix, [{ x: value.SVGViewBoxX + panLimit, y: value.SVGViewBoxY + panLimit }, { x: value.SVGViewBoxX + value.SVGWidth - panLimit, y: value.SVGViewBoxY + value.SVGHeight - panLimit }]),
